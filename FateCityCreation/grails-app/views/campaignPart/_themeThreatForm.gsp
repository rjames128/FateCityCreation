...
<div class="fieldcontain ${hasErrors(bean: campaignPartInstance, field: 'city', 'error')} required">
	...
	<g:select id="city" name="city.id" from="${org.buzzthewindling.City.list()}" optionKey="id" optionValue="cityName" required="" value="${campaignPartInstance?.city?.cityName}" class="many-to-one"/>
</div>
